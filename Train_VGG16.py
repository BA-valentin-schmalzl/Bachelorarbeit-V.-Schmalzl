...
while i < num_slices:
    ...
    df_pred = pd.DataFrame(data={"Filename": img_name_list, "Top Prediction": top_pred_list,
                                 ...
                                 'True_Label': true_label_list, 'Bereich': bereich_list,
                                 'Beschreibung': beschr_list})
    df_pred.to_csv(save_model_path + 'Predicitons{}.csv'.format(i+1), sep=';', index=False, decimal=',')
    del model
    i += 1
...
df_acc.to_csv(save_model_path + 'Acc_Loss.csv', sep=';', index=False, decimal=',')
